...
try {
    // Step 1: Create the Issuer DID
    console.log("Step 1: Creating the Issuer DID...");
    ...
    // Step 2: Set up the DWN (using DIF Community DWN)
    console.log("Step 2: Setting up the DWN...");
    const { web5 } = await Web5.connect({
        didCreateOptions: {
            dwnEndpoints: ['https://dwn.gcda.xyz'],  // Using the community DWN instance
        },
        registration: {
            onSuccess: () => console.log("DWN Creation was successful!"),
            onFailure: (error) => console.error("DWN Creation failed:", error),
        },
    });
    console.log("Successfully done with Step 2.");

    // Step 3: Issue Alice a Known Customer Credential (KCC)
    console.log(issuerDid.uri)
    console.log("Step 3: Issuing a Known Customer Credential (KCC) to Alice...");
    const aliceKcc = await VerifiableCredential.create({
        issuer: issuerDid.uri,  // Issuer's DID URI
   ...
   // console.log("The Alice KCCs ", + aliceKcc)
    // Sign the VC using the issuer DID
    const aliceJWT = await aliceKcc.sign({ did: issuerDid });
    console.log('Issed and Signed Alice Kcc:', aliceKcc);
    console.log("Successfully done with Step 3.");

    // Step 4: Install Protocols on the DWN
    console.log("Step 4: Installing Protocols on the DWN...");
    ...
    // Step 5: Obtain permission to write to Alice's DWN
    console.log("Step 5: Obtaining permission to write to Alice's DWN...");
    ...
    // Step 6: Save the signed VC JWT as a private record in Alice's DWN
    console.log("Step 6: Saving the signed VC JWT as a private record in Alice's DWN...");
    ...
    console.log("Successfully done with Step 6.");
